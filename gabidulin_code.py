...
an encoder based on the generator matrix. It also provides some decoders
- :class:`~sage.coding.gabidulin.GabidulinGaoDecoder` which corrects errors using
  the Gao algorithm in the rank metric.
- :class:`~sage.coding.gabidulin.GabidulinBerlekampWelchDecoder` which corrects errors using
  the classic BerlekampWelch algorithm in the rank metric.
...
from linear_rank_metric import *
...
class GabidulinCode(AbstractLinearRankMetricCode):
    ...
    def __init__(self, base_field, length, dimension, sub_field=None,
        ...
        The dimension is not allowd to exceed the length::

            sage: Fqm = GF(3^40)
            sage: Fq = GF(9)
            sage: n, k = 20, 100
            sage: C = codes.GabidulinCode(Fqm, n, k, Fq)
            Traceback (most recent call last):
            ...
...
class GabidulinVectorEvaluationEncoder(Encoder):
    ...
    def generator_matrix(self):
        ...
        create_matrix_elements = lambda A,k,f: reduce(lambda L,x: chain([x], \
...
class GabidulinBerlekampWelchDecoder(Decoder):
    r"""
    Decoder for Gabidulin codes which uses Berlekamp-Welch
    decoding algorithm from Loidreau's paper to correct
    errors in codewords.
    ...
    EXAMPLES::
        ...
        sage: n, k = 4, 2
        sage: C = codes.GabidulinCode(Fqm, n, k, Fq)
        sage: D = codes.decoders.GabidulinBerlekampWelchDecoder(C)
        sage: D
        Berlekamp-Welch decoder for [4, 2, 3] linear Gabidulin code over GF(6561)/GF(9)

    Alternatively, we can construct the decoder from ``C`` directly::
    ...
    def __init__(self, code):
        if not isinstance(code, GabidulinCode):
            raise ValueError("code has to be a Gabidulin code")
        super(GabidulinBerlekampWelchDecoder, self).__init__(code, code.ambient_space(),
    ...
    def _latex_(self):
        ...
        EXAMPLES:
            ...
            sage: D = codes.decoders.GabidulinBerlekampWelchDecoder(C)
            sage: latex(D)
            \textnormal{Berlekamp-Welch decoder for } [4, 2, 3] \textnormal{ linear Gabidulin code over } \Bold{F}_{3^{8}}/\Bold{F}_{3^{2}}
        """
        return "\\textnormal{Berlekamp-Welch decoder for } %s" % self.code()._latex_()

    def __eq__(self, other):
        """
        Tests equality between GabidulinBerlekampWelchDecoder objects.

        INPUT:

        - ``other`` -- another Gabidulin Berlekamp-Welch Decoder
        ...
        EXAMPLES::
            ...
            sage: n, k = 4, 2
            sage: C = codes.GabidulinCode(Fqm, n, k, Fq)
            sage: D1 = codes.decoders.GabidulinBerlekampWelchDecoder(C)
            sage: D2 = codes.decoders.GabidulinBerlekampWelchDecoder(C)
            sage: D1.__eq__(D2)
            True
            sage: D1 == D2
            True
            ...
            False
        """
        return (isinstance(other, GabidulinBerlekampWelchDecoder)
                and self.code() == other.code()
    ...
    def _decode_to_code_and_message(self, r):
        ...
        EXAMPLES:
            ...
            sage: Chan = channels.StaticRankErrorChannel(C.ambient_space(), D.decoding_radius())
            sage: y = Chan(c)
        ...
        length = len(r)
        eval_pts = C.evaluation_points() # g1, ..., gn
        s = C.twisting_homomorphism()
        R = Fqm['x', s] # skew_polynomials
        t = (C.minimum_distance()-1)//2
        l0 = k+t-1
        l1 = t
        S = matrix(Fqm, n, l0+l1+2,
                   lambda i, j: (s**j)(C.evaluation_points()[i]) if j<(l0+1)
                   else (s**(j-(l0+1)))(r[i]))
        S = S.right_kernel()
        S = S.basis_matrix().row(0)
        N = R(S.list_from_positions(range(l0+1)))
        V = R(S.list_from_positions(range(l0+1, l0+l1+2)))
    ...
    def decode_to_code(self, r):
        ...
        If one tries to decode a word wich is too far from any codeword, an exception is raised::

            sage: Chan2 = channels.StaticRankErrorChannel(C.ambient_space(), D.decoding_radius()+1)
            sage: y = Chan2(c)
    ...
    def decode_to_message(self, r):
        ...
        - the message corresponding to the received codeword (a skew polynomial ?)
    ...
    def decoding_radius(self):
        """
        Return the decoding radius of the Gabidulin Berlekamp Welch Decoder.

        OUTPUT:

        - the maximal rank of an error, as an integer
...
class GabidulinRightBerlekampWelchDecoder(Decoder):
    r"""
    Decoder for Gabidulin codes which uses right side Berlekamp-Welch
    decoding algorithm errors in codewords.
    ...
    def __init__(self, code):
        if not isinstance(code, GabidulinCode):
            raise ValueError("code has to be a Gabidulin code")
        super(GabidulinRightBerlekampWelchDecoder, self).__init__(code, code.ambient_space(),
                                                                 "PolynomialEvaluation")
    ...
    def _repr_(self):
        ...
        EXAMPLES:
            ...
            TODO
        """
        return "Right hand side Berlekamp-Welch decoder for %s" % self.code()
    ...
    def _latex_(self):
        ...
        EXAMPLES:
            ...
            TODO
        """
        return "\\textnormal{Right hand side Berlekamp-Welch decoder for } %s" % self.code()._latex_()

    def __eq__(self, other):
        """
        Tests equality between GabidulinRightBerlekampWelchDecoder objects.
        ...
        EXAMPLES::
            ...
            TODO
        """
        return (isinstance(other, GabidulinRightBerlekampWelchDecoder)
                and self.code() == other.code()
                and self.input_space() == other.input_space())
    ...
    def _decode_to_code_and_message(self, r):
        ...
        Fqm = C.base_field()
        Fq = C.sub_field()
        m = C.base_field().degree() // C.sub_field().degree()
        if r not in C.ambient_space():
            raise ValueError("The word to decode has to be in the ambient space of the code")
        n, k = C.length(), C.dimension()
        if n == k:
        ...
        t = (C.minimum_distance() - 1) // 2
        points = [(eval_pts[i], r[i]) for i in range(len(eval_pts))]
        Y = reduction(R.lagrange_polynomial(points), m)
        Ystar = adjoint(Y, m)
        rstar = Ystar.multi_point_evaluation(eval_pts)  # Need some padding here
        pts = [(eval_pts[i], rstar[i]) for i in range(len(eval_pts))]
        l0 = k+t-1
        l1 = t
        S = matrix(Fqm, n, l0+l1+2,
                   lambda i, j: (s**(m-j))(C.evaluation_points()[i]) if j<(l0+1)
                   else (s**(m-j+(l0+1)))(rstar[i]))
        S = S.right_kernel()
        S = S.basis_matrix().row(0)
        Nstar_coeff = list(reversed(S.list_from_positions(range(l0+1))))
        Vstar_coeff = list(reversed(S.list_from_positions(range(l0+1, l0+l1+2))))
        Nstar = R(Nstar_coeff) << m-l0 # shift
        Vstar = R(Vstar_coeff) << m-l1 # shift
        N = adjoint(Nstar, m)
        V = adjoint(Vstar, m)
        f, rem = (-N).right_quo_rem(V)
        if not rem.is_zero():
            raise DecodingError("Decoding failed because the number of errors exceeded the decoding radius")
        if f not in R:
            raise DecodingError("Decoding failed because the number of errors exceeded the decoding radius")
...
GabidulinCode._registered_decoders["BerlekampWelch"] = GabidulinBerlekampWelchDecoder
GabidulinCode._registered_decoders["RightBerlekampWelch"] = GabidulinRightBerlekampWelchDecoder
